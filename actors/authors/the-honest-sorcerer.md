...
Resource depletion is exponential — increasingly harder to access, more energy-intensive to extract. This is not a linear decline but an accelerating one: the easy resources are extracted first, and each subsequent unit requires more energy to obtain than the last. The energy return on investment (EROI) declines structurally, meaning that even as gross energy production holds, net energy available to society contracts.

## "There Is No Next Economy"

The most provocative claim: economic growth cannot be maintained by switching to renewables on a finite planet with depleting resources. The framework argues that the "next economy" — a hypothetical post-fossil-fuel growth paradigm — does not exist and cannot exist. Renewables are not a replacement for fossil fuels but an extension of the fossil fuel system, because their manufacture, maintenance, and replacement depend on fossil-energy inputs.

## Circular Economy Limits

The circular economy cannot restore material growth because recycling always loses material. Thermodynamic limits — entropy — guarantee that no recycling process is complete. This means that a finite planet with continuous extraction and incomplete recycling is on a one-way trajectory of material depletion, regardless of efficiency gains.

## Electricity Is Auxiliary

Electricity is auxiliary, not a primary energy source. Renewables are an extension of the fossil fuel system, not a replacement. This distinction matters because it means that "electrification" does not equal "decarbonization" — the primary energy inputs that build and maintain the electrical infrastructure remain fossil-based.
...
The "megamachine" and "Wetiko" concepts frame the growth imperative as a civilizational pathology — a system that consumes itself in the pursuit of expansion. Wetiko, borrowed from indigenous North American philosophy, names a cannibalistic logic: the system feeds on its own resource base to sustain growth, consuming the conditions of its own existence.
