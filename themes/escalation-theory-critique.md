...
2. **Non-unitary actors**: The rational-actor model treats states as unitary agents. In practice, the US-Iran war was [Trump](/actors/trump.md)'s personal decision, not a state-level rational calculation — as [O'Brien](/actors/authors/phillips-obrien.md) demonstrates. Iran's decision-making involves the IRGC, the Artesh, the pragmatists, and the Supreme Leader's faction — not a single utility-maximizing agent [1][3].
...
[Aurelien](/actors/authors/aurelien.md) does not propose an alternative formal model but argues for a historical-analogical approach: understanding conflicts through their specific historical contexts, the particular actors involved, and the irreducible complexity of their objective structures. This approach is less predictive but more accurate — it does not generate the false precision of formal models but it also does not generate their false predictions [1].

The critique aligns with [O'Brien](/actors/authors/phillips-obrien.md)'s leader-centered framework: if wars are choices of individual leaders, not state-level rational calculations, then models that assume state-level rationality will systematically fail to predict when and how wars occur. The critique also aligns with [Tchakarova](/actors/authors/velina-tchakarova.md)'s simultaneity framework: if multiple crises interact in ways that no single model can capture, then models that analyze each crisis in isolation will miss the systemic dynamics that drive the [global system rupture](/themes/global-system-rupture.md) [1][3][4].
